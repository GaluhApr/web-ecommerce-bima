...
<div class="container">
    <div class="row d-flex align-items-center ">
        ...
        @php
        $Products = App\Models\Products::where('category', $id)->get();
        @endphp

        <!-- Products Starts Here -->
        <section id="Products" align="center" class="px-5" style=" font-family: 'Balsamiq Sans', cursive;">

            <h1 class="black-text" style="font-weight:bold;">{{ App\Category::find($id)->name }}</h1>
            ...
            <ul class="list-group">
                <li class="list-group-item"><a href="{{ route('allproducts') }}">all</a></li>
                @foreach ($categories as $category)
                <li class="list-group-item {{$category->id == $id ? 'active' : '' }}"> <a href="{{ route('category', ['id' => $category->id]) }}">{{ $category->name }}</a>
                </li>
                @endforeach
            </ul>
